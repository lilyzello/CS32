    ...
    for (;;)
    {
        ...
        else if (playerMove.size() == 1)
        {
            if (tolower(playerMove[0]) == 'c')
                return player->dropPoisonedCarrot();
            //new
            //displays the arena's history grid, followed by continue statement, if player types 'h'
            else if (tolower(playerMove[0]) == 'h'){
               (m_arena->history()).display();
                cout << "Press enter to continue" << endl;
                cin.ignore(100000, '\n');
                return "\0"; //function must return a string, so returns the null character
...
void Game::play()
{
    ...
    while ( ! player->isDead()  &&  m_arena->rabbitCount() > 0)
    {
        ...
        //new
        if (msg != "\0") //if the player did not type 'h'
        {
            m_arena->display(msg);
            if (player->isDead())
                break;
        ...
        else if (msg == "\0") //if the player typed 'h'
        {
            m_arena->display(msg);
            if (player->isDead())
                break;
